    ...
    <div v-else-if="videos.length > 0" class="p-4">
      <div class="grid grid-cols-2 gap-3">
        <div
          ...
          :key="video.id"
          class="relative group rounded-xl overflow-hidden bg-[#1a1a22] cursor-pointer"
          @click="router.push({ name: 'user-profile', params: { id: video.author.id } })"
        >
          <!-- 封面 -->
