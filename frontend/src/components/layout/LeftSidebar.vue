...
<template>
  <aside class="flex flex-col w-60 shrink-0 h-full bg-[#111115] overflow-y-auto scrollbar-none">
    ...
    <div class="px-5 pt-6 pb-5 shrink-0">
      <span class="text-2xl font-black tracking-tight bg-gradient-to-r from-violet-400 via-purple-300 to-fuchsia-300 bg-clip-text text-transparent">
        闪视
      </span>
    </div>
    ...
    <div class="px-5 pb-5 shrink-0">
      <div class="flex flex-wrap gap-x-2.5 gap-y-1 text-[11px] text-gray-600">
        <span v-for="link in ['关于','招聘','帮助','条款','隐私']" :key="link"
          class="hover:text-gray-400 cursor-pointer transition-colors">{{ link }}</span>
      </div>
