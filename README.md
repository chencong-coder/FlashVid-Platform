...
• 设计统一计数更新架构：所有统计计数（点赞/收藏/评论/观看）在业务层同步更新 Redis，保证实时性；通过 RabbitMQ 异步触发热度更新和 MySQL 批量同步，降低 DB 写压力 90%+，点赞/收藏接口 QPS 提升至 396-730

• 实现无锁分布式并发控制：利用 Redis SADD 原子操作返回值（0/1）作为天然分布式锁，避免传统 SIsMember + SADD 两步操作的竞态窗口，保证点赞/收藏幂等性，事务失败自动回滚 Redis 状态

• 利用 Redis 缓存 + 时间衰减算法实现视频热度排序，配合缓存三大问题防护（空值缓存防穿透、singleflight 防击穿、TTL 随机偏移防雪崩），视频详情接口 QPS 达 1090（性能提升 495 倍），话题热榜 QPS 达 3069

• 搭建消息可靠性保障机制：Producer Confirm 模式保证发送成功，Consumer 手动 ACK + 最多重试 3 次 + 死信队列托底，保障通知与热度更新的最终一致性
