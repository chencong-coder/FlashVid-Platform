...
本接口已从 MySQL 随机查询优化为 **Redis ZSet 热榜推荐**，大幅提升推荐效率和用户体验。

| 对比项 | 旧实现（MySQL） | 新实现（Redis ZSet） |
|-------|----------------|---------------------|
| **查询方式** | `ORDER BY RAND() LIMIT 10` | `ZREVRANGE video:hot 0 9` |
| **数据源** | MySQL 全表扫描 | Redis 内存有序集合 |
| **推荐策略** | 完全随机，无质量保证 | 基于热度分数排序，优先推荐高质量内容 |
| **查询性能** | 数万条数据扫描，耗时 50-200ms | 内存直接返回，耗时 < 5ms |
| **并发能力** | MySQL 压力大，易成为瓶颈 | Redis 高并发支持，QPS 10万+ |
| **实时性** | 静态数据，不反映当前热度 | 实时更新热度分数，推荐始终最新 |

**热度分数计算公式**：
```
热度分数 = 播放量×1 + 点赞数×5 + 收藏数×8 + 评论数×10
```
...
- 分数: 热度分数（浮点数）
- 更新方式: `ZINCRBY` 异步增量更新
- 查询方式: `ZREVRANGE` 倒序获取 Top N

**初始化工具**：
- 命令: `go run cmd/init-hot/main.go`
- 功能: 从 MySQL 读取所有视频并计算热度分数，批量写入 Redis
- 适用场景: 首次部署或 Redis 数据丢失后重建
